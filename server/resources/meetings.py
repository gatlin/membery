...
class Meetings(Resource):
    ...
    def list(self):
        ...
        argparser.add_argument('committee', type=int,
                               help='Filter by committee')
        ...
        if 'committee' in args:
            cur.execute('select exists (select 1 from committees where id = %s)',
                        (args['committee'],))
            q = '''
            select id, committee, name, start_time, end_time from meetings
            where committee = %s
            '''
            if cur.fetchone()[0]:
                cur.execute(q, (args['committee'],))
            else:
    ...
    def put(self, meeting_id=None):
        ...
        where id = %s
        '''
        cur.execute('update meetings set name = %s where id = %s', (
            body['committee'],
            body['name'],
            body['start_time'],
            body['end_time'],
